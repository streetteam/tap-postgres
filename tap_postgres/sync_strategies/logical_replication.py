...
def locate_replication_slot(conn_info):
    with post_db.open_connection(conn_info, False) as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                "SELECT * FROM pg_replication_slots WHERE slot_name = %s AND plugin = %s",
            ...
            raise Exception(
                "Unable to find replication slot (stitch || {} with wal2json".format(
                    db_specific_slot
                )
            )
...
def sync_tables(conn_info, logical_streams, state, end_lsn):
    ...
    poll_total_seconds = (
        conn_info["logical_poll_total_seconds"] or 60 * 30
    )  # we are willing to poll for a total of 30 minutes without finding a record
            ...
            while True:
                ...
                if msg:
                    ...
                    state = consume_message(
                        logical_streams, state, msg, time_extracted, conn_info, end_lsn
                    )
                    ...
                    try:
                        ...
                        if not any(sel):
                            LOGGER.info(
                                (
                                    "no data for {} seconds. sending feedback to server with NO "
